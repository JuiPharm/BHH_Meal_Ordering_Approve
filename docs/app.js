 ...
 * Uses GAS API via fetch()
 */
const CFG = window.APP_CONFIG || {};
const API_BASE = (CFG.API_BASE_URL || "").trim();

function assertConfig(){
  if (!API_BASE || API_BASE.includes("PASTE_YOUR_GAS_WEB_APP_URL_HERE")){
    document.body.innerHTML = `
      <div style="max-width:720px;margin:40px auto;font-family:system-ui;padding:16px">
        <h2>ยังไม่ได้ตั้งค่า API_BASE_URL</h2>
        <p>กรุณาแก้ไฟล์ <code>docs/config.js</code> แล้วใส่ Google Apps Script Web App URL</p>
      </div>`;
...
function actionCell(status){
  ...
  if(isPending(s)){
    return `<div class="small text-warning mb-1"><strong>🔔 รอ Food House รับ Order</strong></div>
      <button class='btn btn-warning btn-sm' data-action="step0"><i class="fas fa-check"></i> รับออเดอร์</button>${slipBtn}`;
  }
  if(s.includes("Food House รับ Order")){
    return `<div class="small text-info mb-1"><i class="fas fa-clock"></i> กำลังเตรียมอาหาร</div>
      <button class='btn btn-primary btn-sm' data-action="step1"><i class="fas fa-utensils"></i> เตรียมเสร็จ</button>${slipBtn}`;
  }
  if(s.includes("Food House เตรียมอาหารเสร็จแล้ว")){
    return `<div class="small text-success mb-1"><i class="fas fa-truck"></i> รอส่งให้หน่วยงาน</div>
      <button class='btn btn-success btn-sm' data-action="step2"><i class="fas fa-check-circle"></i> หน่วยงานรับแล้ว</button>${slipBtn}`;
  }
  return `<div class="small text-success"><i class="fas fa-check-double"></i> ส่งสำเร็จแล้ว</div>${slipBtn}`;
}
function buildTable(rows){
  const mainHeaders=["Action","ID","สถานะ","วันที่","HN","ชื่อผู้ป่วย","วันเกิด","แพ้อาหาร","โรคประจำตัว","ผู้สั่ง","แผนก","แซนวิชทูน่า","ข้าวต้มปลา","ข้าวต้มไก่","ข้าวต้มกุ้ง","เมนู Custom","รายละเอียดอื่น"];
...
async function refreshPending(){
  ...
  showIndicator(count);
  if(count>lastPending){ const ok=await playOnce(); if(!ok && document.hidden) notifyOS(count); }
  lastPending=count; scheduleAlarm();
}
async function versionLoop(){
  try{
    const d=await apiGet("version");
    const v=Number(d.version||0);
    if(v!==lastVersion){ lastVersion=v; await refreshAll(); await refreshPending(); }
  }catch(e){}
